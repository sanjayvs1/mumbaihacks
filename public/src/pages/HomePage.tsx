    ...
    <div className="min-h-screen bg-blue-50 flex flex-col items-center font-sans text-gray-800">
      ...
      <section className="flex flex-col md:flex-row items-center justify-center py-16 px-8 space-y-8 md:space-y-0 md:space-x-12 bg-blue-50">
        <div className="w-full max-w-md">
          <img src={illustration} alt="Meeting Illustration" className="rounded-lg shadow-lg" />
        </div>
        <div className="max-w-lg text-center md:text-left">
          <h2 className="text-5xl font-extrabold text-blue-900 mb-4">AI-Driven Meeting Assistant</h2>
      ...
      <section id="features" className="w-full py-16 px-8 bg-white">
        <h3 className="text-3xl font-bold text-center text-blue-900 mb-10">Features</h3>
        <div className="flex flex-wrap justify-center gap-6 max-w-5xl mx-auto">
          {["Real-Time Transcription", "Summarized Minutes", "Automated Clips", "Task Reminders"].map((feature, idx) => (
            <div
              key={idx}
            ...
            >
              ...
              <p className="text-gray-600">
                {feature === "Real-Time Transcription" && "Get instant transcriptions for every meeting."}
                {feature === "Summarized Minutes" && "Receive AI-generated minutes with key highlights."}
                {feature === "Automated Clips" && "Access video clips of essential meeting parts."}
                {feature === "Task Reminders" && "Receive reminders for key follow-up tasks."}
              </p>
            </div>
          ))}
        </div>
      ...
      <footer className="w-full py-6 bg-blue-900 text-white text-center">
        <p className="text-sm">&copy; {new Date().getFullYear()} MeetingMate. All rights reserved.</p>
      </footer>
    </div>
  );
};
